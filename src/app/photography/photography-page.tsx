...
/** Deterministic PRNG (mulberry32) — same output on server and client, avoiding hydration mismatches. */
...
function spreadShuffle<T extends { alt: string; project?: string }>(arr: T[]): T[] {
  ...
  const buckets = [...groups.values()].map((g) => shuffle(g, rng));
...
export default function PhotographyPage() {
  ...
  const featuredPhoto =
    galleryPhotos.find((p) => p.src.includes("IMAGE_-06_fed37a")) ||
    galleryPhotos.find((p) => p.aspect === "wide") ||
    galleryPhotos[0];
